...
module.exports.controller = (app, io, socket_list ) => {
    ...
    app.post('/api/admin/order_detail', (req, res) => {
        ...
        checkAccessToken(req.headers, res, (userObj) => {
            ...
            , [reqObj.order_id, reqObj.user_id, reqObj.order_id, reqObj.user_id], (err, result) => {
                if(err){
                    helper.ThrowHtmlError(err, res)
                    return
                }
            ...
            helper.CheckParameterValid(res, reqObj, ["order_id", "user_id", "order_status"], () =>{
                db.query("UPDATE `order_detail` SET `order_status`= ?,`modify_date`= NOW() WHERE `order_id` = ? AND `user_id` = ? AND `order_status` < ? ", [reqObj.order_status, reqObj.order_id, reqObj.user_id, reqObj.order_status] , (err, result) =>{
                    ...
                    if(result.affectedRows >0){
                        ...
                        var message = ""
                        var notiType = 2
                        //1:new, 2: order_accept, 3: order_deliverd, 4: cancel, 5: order_declined
                        ...
                        db.query("INSERT INTO `notification_details`(`ref_id`, `user_id`, `title`, `message`, `notification_type`) VALUES (?,?,?, ?,?)",[ reqObj.order_id, reqObj.user_id, title, message, notiType],(err, iResult) => {
                            if(err){
